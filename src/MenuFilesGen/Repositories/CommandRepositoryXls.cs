        ...
        public void ReadFromXls()
        {
            ...
            foreach (IXLRow row in rows)
            {
                ...
                CommandDefinition res = new CommandDefinition
                {
                    ...
                    ResourceDllName = row.Cell(columnNumbers.ResourseDllColumn + 1).GetString().Trim(),
                    ...
                    IsVirtualPanel = row.Cell(columnNumbers.IsVirtualPanelColumn + 1).GetString().Contains("ИСКЛЮЧИТЬ", StringComparison.OrdinalIgnoreCase),//not used
